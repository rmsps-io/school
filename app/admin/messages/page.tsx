  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">{messages?.length ?? 0} messages</p>
      </div>
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <MessageSquare className="h-4 w-4 text-primary" /> Inbox / Sent
          </CardTitle>
        </CardHeader>
        <CardContent>
          {!messages || messages.length === 0 ? (
            ...
            <div className="space-y-2">
              {messages.map((m) => {
                ...
                const isOwn    = m.sender && (m.sender as {full_name:string}[])?.[0]?.full_name !== '—'
                return (
                  <div key={m.id} className={`rounded-lg border p-3 ${!m.is_read ? 'bg-primary/5 border-primary/20' : ''}`}>
                    <div className="flex items-center justify-between gap-2 flex-wrap mb-1">
                      <p className="text-xs text-muted-foreground">
                        <span className="font-medium text-foreground">{sender}</span> → {receiver}
                      </p>
